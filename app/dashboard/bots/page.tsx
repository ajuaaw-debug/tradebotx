...
export default async function BotsPage() {
  ...
  const bots = dbUser?.botInstances ?? [];
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
        ...
        <a href="/dashboard/marketplace" style={{ background: "#7c3aed", color: "white", padding: "10px 20px", borderRadius: 8, fontSize: 14, fontWeight: 500, textDecoration: "none" }}>
          + New Bot
        </a>
      </div>
      ...
      {bots.length === 0 ? (
        <div style={{ background: "#0f0f0f", border: "1px solid #1f2937", borderRadius: 12, padding: 48, textAlign: "center" }}>
          ...
          <a href="/dashboard/marketplace" style={{ background: "#7c3aed", color: "white", padding: "10px 24px", borderRadius: 8, fontSize: 14, fontWeight: 500, textDecoration: "none" }}>
            Browse Marketplace
          </a>
        </div>
      ) : (
          ...
          {bots.map((bot) => (
            <div key={bot.id} style={{ background: "#0f0f0f", border: "1px solid #1f2937", borderRadius: 12, padding: 24, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr auto", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <a href={`/dashboard/bots/${bot.id}`} style={{ background: "#1f2937", border: "none", color: "white", padding: "8px 14px", borderRadius: 6, fontSize: 13, textDecoration: "none" }}>
                  View
                </a>
              </div>
            </div>
